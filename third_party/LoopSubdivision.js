...
class LoopSubdivision {
  ...
  static flat(geometry, params = {}) {
    ...
    attributeList.forEach((attributeName) => {
      ...
      loop.setAttribute(
        attributeName,
        LoopSubdivision.flatAttribute(attribute, vertexCount, params)
      );
    });
    ...
    for (const attributeName in morphAttributes) {
      ...
      for (let i = 0, l = morphAttribute.length; i < l; i++) {
        if (morphAttribute[i].count !== vertexCount) continue;
        array.push(
          LoopSubdivision.flatAttribute(morphAttribute[i], vertexCount, params)
        );
      }
  ...
  static smooth(geometry, params = {}) {
    ...
    for (const attributeName in morphAttributes) {
      ...
      for (let i = 0, l = morphAttribute.length; i < l; i++) {
        ...
        const flattenedAttribute = LoopSubdivision.flatAttribute(
          morphAttribute[i],
    ...
    function subdivideAttribute(
      ...
      const arrayLength =
        flat.attributes.position.count * flattenedAttribute.itemSize;
